...
const handleSignUp = async (e: Event) => {
	...
	if (!allMet)              { error = 'Your password does not meet all the requirements yet.'; return }
		...
		<div class="form-inner">
			...
			<form novalidate onsubmit={handleSignUp}>
				...
				<div class="field">
					...
					{#if password.length > 0}
						<div class="strength">
							<div class="bars">
								{#each [0,1,2,3] as i}
									<span class="bar" style="background:{i < barFill ? barColor : '#E6E4DC'}"></span>
								{/each}
							</div>
							<div class="strength-meta">
								<span style="color:{barColor};font-weight:500">{barLabel}</span>
								<span class="muted">{metCount}/3</span>
							</div>
							<ul class="req-list">
								{#each reqs as req}
									...
									<li class:met={ok}>
										...
										{:else}
											<span class="req-dot req-off"></span>
										{/if}
										{req.label}
									</li>
								{/each}
				...
				<div class="field">
					...
					{#if confirm.length > 0 && !pwdMatch}
						<span class="field-err">Passwords do not match.</span>
					{:else if confirm.length > 0 && pwdMatch}
						<span class="field-ok">Passwords match.</span>
					{/if}
				</div>
...
.panel-headline {
	...
	letter-spacing: -0.6px; color: #fff; margin-bottom: 14px;
}
.panel-sub {
...
.strength-meta { display: flex; justify-content: space-between; font-size: 11px; }
...
.req-list li {
	...
	font-size: 12px; color: var(--warm-text-tertiary); transition: color 0.15s;
}
...
.req-ok  { background: var(--teal-light); }
.req-off { border: 1px solid var(--warm-border); }

/* Inline field status */
.field-err { font-size: 12px; color: #ef4444; }
.field-ok  { font-size: 12px; color: #16a34a; font-weight: 500; }
...
.terms {
	font-size: 11px; color: var(--warm-text-tertiary);
