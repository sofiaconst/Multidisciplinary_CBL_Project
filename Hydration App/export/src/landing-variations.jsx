...
// In the bundled build, every (name,color) pair used is registered as a
// <meta name="ext-resource-dependency"> so the bundler inlines it.
// window.__resources holds the inlined data-URLs keyed by resource id.
// Pattern: "icon__<name>__<colorTagWithoutHash>".
function Icon({ name, size = 20, color = 'currentColor', style }) {
  const tag = String(color).replace('#', '').toLowerCase();
  const key = `icon__${name}__${tag}`;
  const bundled = typeof window !== 'undefined' && window.__resources && window.__resources[key];
  const url =
    bundled ||
